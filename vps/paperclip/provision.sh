#!/bin/bash
# Paperclip Platform — Full automated provisioning
#
...
#   - vps/paperclip/.env.production with real secrets
#
# Usage:
#   cd wopr-ops && bash vps/paperclip/provision.sh
#
# To destroy and recreate:
#   DESTROY_FIRST=true bash vps/paperclip/provision.sh
...
CF_ZONE="c2ac899c5e55d3ac150197a18effadf2"  # runpaperclip.com
DROPLET_NAME="paperclip-platform"
REGION="sfo2"
SIZE="s-1vcpu-1gb"
IMAGE="ubuntu-24-04-x64"
SSH_KEY_IDS="54912818,52980537"
DOMAINS=("runpaperclip.com" "api.runpaperclip.com" "app.runpaperclip.com")
...
if [ "${DESTROY_FIRST:-}" = "true" ]; then
  ...
  EXISTING=$(do_api GET "/droplets?tag_name=paperclip" | jq -r '.droplets[] | select(.name=="'"$DROPLET_NAME"'") | .id')
...
echo "==> Creating droplet ($SIZE — Paperclip platform)..."
...
heredoc = \"cat > /opt/paperclip-platform/.env << 'ENVEOF'\n\" + env.strip() + \"\nENVEOF\"
...
DROPLET_ID=$(do_api POST "/droplets" \
  -d "$(jq -n \
    ...
    '{name:$name,region:$region,size:$size,image:$image,ssh_keys:$ssh_keys,user_data:$user_data,tags:["paperclip","platform","production"]}'
  )" | jq -r '.droplet.id')
...
for domain in "${DOMAINS[@]}"; do
  ...
  if [ -n "$RECORD_ID" ]; then
    ...
    SUBDOMAIN="${domain%%.*}"
    [ "$domain" = "runpaperclip.com" ] && SUBDOMAIN="@"
...
for i in $(seq 1 40); do
  STATUS=$(ssh -o StrictHostKeyChecking=no -o ConnectTimeout=5 root@"$IP" \
    'cat /var/log/cloud-init-paperclip.log 2>/dev/null | head -1' 2>/dev/null || true)
  if [[ "$STATUS" == PAPERCLIP_PLATFORM_READY* ]]; then
...
echo "$IP" | gh secret set PROD_HOST --repo wopr-network/paperclip-platform
echo "$DEPLOY_KEY" | gh secret set PROD_SSH_KEY --repo wopr-network/paperclip-platform
echo "$IP" | gh secret set STAGING_HOST --repo wopr-network/paperclip-platform-ui
echo "$DEPLOY_KEY" | gh secret set SSH_DEPLOY_KEY --repo wopr-network/paperclip-platform-ui
...
HEALTH=$(curl -sf "https://api.runpaperclip.com/health" 2>/dev/null || echo "unreachable")
echo "    Health: $HEALTH"

# --- Done ---
echo ""
echo "=== Paperclip Platform Deployed ==="
echo "  Droplet: $DROPLET_ID ($IP)"
echo "  DNS:     runpaperclip.com, api.runpaperclip.com, app.runpaperclip.com"
echo "  TLS:     Let's Encrypt via Caddy"
echo "  SSH:     ssh root@$IP / ssh deploy@$IP"
echo ""
echo "To redeploy from scratch:"
echo "  DESTROY_FIRST=true bash vps/paperclip/provision.sh"
